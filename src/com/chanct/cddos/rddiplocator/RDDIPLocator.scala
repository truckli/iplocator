...
class RDDIPLocator(sc: SparkContext, fileName: String) extends Serializable {
  ...
  def locateRDD(longIPRDD: RDD[Long]): RDD[(Long, IPLocation)] = {
    longIPRDD.map { longIP => (longIP, locate(longIP)) }
  }

  // double definition
  def locateRDD(strIPRDD: => RDD[String]): RDD[(String, IPLocation)] = {
    strIPRDD.map { strIP => (strIP, locate(strIP)) }
  }

  private def locate(longIP: Long): IPLocation = {
  ...
  private def locate(strIP: String): IPLocation = {
    val ipRegion: Array[String] = new Array[String](7)
